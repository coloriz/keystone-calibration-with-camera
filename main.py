...
def read_one_frame_from_camera():
    ...
    return cv2.cvtColor(image, cv2.COLOR_BAYER_BG2BGR_EA)
...
def find_screen_and_projection_area(_img):
    ...
    img = cv2.bilateralFilter(img, 11, 30, 40)
    if DEBUG:
        imshow_and_wait(img, 'bilateralFilter')

    img = cv2.Canny(img, 5, 30)
    if DEBUG:
        imshow_and_wait(img, 'Canny')

    img = cv2.morphologyEx(img, cv2.MORPH_CLOSE, np.ones((7, 7), np.uint8), iterations=3)
    if DEBUG:
...
def find_screen_and_projection_area2():
    ...
    cv2.waitKey(100)
    distorted = read_one_frame_from_camera()
...
img_yosemite = cv2.imread('images/ZtcjR.jpg')


while True:
    M = get_transformation_matrix()
    y = cv2.warpPerspective(img_yosemite, M, (DISPLAY_WIDTH, DISPLAY_HEIGHT))
    cv2.rectangle(y, (0, 0), (DISPLAY_WIDTH - 2, DISPLAY_HEIGHT - 2), (0, 0, 255), 2)
    cv2.imshow(WINDOW_NAME, y)
